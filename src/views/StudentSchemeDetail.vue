            ...
            <div class="flex flex-col gap-2 flex-1">
              ...
              <div class="w-full flex-1 min-h-[80px] bg-[#1a1d24] border border-gray-700 rounded-lg p-3 text-sm text-gray-200 disabled:opacity-60">
                ...
                <div v-else-if="currentGroupId === 2">
                  <p class="mb-2"><span class="text-green-400 font-bold">✓ 优势：</span>侧信道防护措施综合考虑性能与效率，采用一阶掩码机制实现，能够高效抵御DPA攻击。</p>
...
const groups = reactive([
  ...
  },
  {
    ...
    name: '第三组',
    title: '通信安全 + 抗重放攻击',
    subtitle: '抗重放核心防护体制', // 9个字，符合十字以内要求
    codeId: 'SEC-NOREPLAY-03',
    themeColor: '#f59e0b', 
    desc: '基于滑动窗口与严格同步计数机制，精准拦截并丢弃所有重放的旧数据包，彻底阻断指令重放攻击。',hardware: 'STM32L432',
    archLayers: [
    ...
    algorithms: [
      { label: '通信安全加密', name: 'AES 算法', desc: '硬件实现简单、资源占用低，对控制指令、航拍与定位数据进行全程加密传输。' },
  ...
  for (let targetId of targetGroups) {
    ...
    if (!group.review.isSubmitted) {
      ...
      const reviewComments = {
        ...
        2: '侧信道防护措施综合考虑性能与效率，采用一阶掩码机制实现，能够高效抵御DPA攻击。防护措施可能会增加系统复杂度和功耗，需要在安全性和性能之间找到平衡。',
